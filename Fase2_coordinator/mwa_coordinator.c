...
info_node_t nodes[MAX_NODES] = {0};
static uint8_t node_index;

uint16_t check_prev_node(uint64_t Source_Address)
{
	uint8_t nodeIndex = 0;

		for (nodeIndex = 0; nodeIndex < MAX_NODES ; nodeIndex++)
		{
			if (nodes[nodeIndex].Extended_Address == Source_Address)
			{
				return nodes[nodeIndex].Short_Address;  /** The node has joined previously return short address*/
			}
		}

		/** If there's no coincidence assign a new short address**/
		return  NULL_NODE;
}


void save_node_info(info_node_t node_info)
{

	if(node_index < (MAX_NODES-1)) /** Check if there's available space*/
	{
		nodes[node_index].Extended_Address = node_info.Extended_Address;
		nodes[node_index].Short_Address = node_info.Short_Address;
		nodes[node_index].RxOnWhenIdle = node_info.RxOnWhenIdle;
		nodes[node_index].DeviceType = node_info.DeviceType;
		node_index++;
	}
	else
	{
		node_index = 0;
		Serial_Print(interfaceId, "\n\rCouldn't add node , max number of nodes reach\n\r", gAllowToBlock_d);
	}
...
static uint8_t App_SendAssociateResponse(nwkMessage_t *pMsgIn, uint8_t appInstance)
{
  ...
  static uint16_t short_address = 0x0001;
  uint16_t return_short_address;
  info_node_t  joining_node_info;

  joining_node_info.Extended_Address = pMsgIn->msgData.associateInd.deviceAddress;
  joining_node_info.Short_Address    = short_address;
  joining_node_info.RxOnWhenIdle     = (pMsgIn->msgData.associateInd.capabilityInfo && gCapInfoRxWhenIdle_c);
  joining_node_info.DeviceType       = (pMsgIn->msgData.associateInd.capabilityInfo && gCapInfoDeviceFfd_c);
  ...
  if(pMsg != NULL)
  {
    ...
    if(pMsgIn->msgData.associateInd.capabilityInfo & gCapInfoAllocAddr_c)
    {
      ...
      //pAssocRes->assocShortAddress = 0x0001;
    	...
    	return_short_address = check_prev_node(pMsgIn->msgData.associateInd.deviceAddress);

		if(return_short_address != NULL_NODE)
		{
			pAssocRes->assocShortAddress = return_short_address;  /** put the old short address previously assigned*/
			Serial_Print(interfaceId,"\n\rPreviously Connected Device! Old Address Assigned -> 0x", gAllowToBlock_d);
			Serial_PrintHex(interfaceId, (uint8_t*)&return_short_address, 2, gPrtHexNoFormat_c);

		}
		else
		{
			save_node_info(joining_node_info);
			pAssocRes->assocShortAddress = short_address;	/** Assign a new short address*/
			Serial_Print(interfaceId,"\n\r New Connected Device! New Address Assigned -> 0x", gAllowToBlock_d);
			Serial_PrintHex(interfaceId, (uint8_t*)&short_address, 2, gPrtHexNoFormat_c);
			short_address++;
			if(short_address >= MAX_NODES)
				short_address = 0;
		}
    }
    else
...
static void App_HandleMcpsInput(mcpsToNwkMessage_t *pMsgIn, uint8_t appInstance)
{
    ...
    /* The MCPS-Data indication is sent by the MAC to the network
       ...
       copy the received data to the UART. */
		...
		rcv_counter = pMsgIn->msgData.dataInd.pMsdu[8] - '0';
